...
public abstract class AbstractEventCommonService {
     ...
     */
    protected LocalDateTime parseOrSetDefaultTime(String str, LocalDateTime timeDef) {
        return timeDeserializer.parseOrSetDefaultTime(str, timeDef);
    }
    ...
    /**
     * вспомогательный метод проверки-присвоения нового времени события при обновлении события<p>
     * используется в сервис-слое для API администратора, а также приватного API<p>
     * проверяет условие, что событие отстоит от момента обновления на заданных промежуток времени согласно ТЗ<p>
     * @implNote - согласно ТЗ при обновлении события в приватном API, дата и время на которые намечено событие
     *           не может быть раньше, чем через два часа от текущего момента (Ожидается HTTP-статус CONFLICT: 409)<p>
     *           - согласно предоставленным Postman-тестам при попытке изменения даты события на уже наступившую
     *           (реализуется указанием аргумента метода checkPermittedTimeDatesRelation throwBadRequest)
     *           ожидается HTTP-статуса BAD REQUEST: 400
     */
    protected LocalDateTime checkAndRetrieveNewEventDate(LocalDateTime persistedEventDate, EventInputDto dto) {
        LocalDateTime newEventDate = parseOrSetDefaultTime(dto.getEventDate(), persistedEventDate);
